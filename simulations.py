...
from coverage_controll_triangle_2 import CoverageControllerTriangle
...
def show_coverage_triangle(n):
    ...
    def update(frame_number):
        if coverage_controller.is_completely_covered():
            ...
            ax.set_title('number of circles = {}'.format(coverage_controller.number_of_circles))
        yield ax.add_patch(co.__next__())

    anim = FuncAnimation(fig, update, interval=1, save_count=100, blit=True)
    plt.show()

def test_frequencies_triangle(n, simulations):
    results1 = []
    results2 = []
    for i in range(0, simulations):
        coverage_controller = CoverageControllerTriangle(n)
        result = coverage_controller.run_simulation()
        results1.append(result[0])
        results2.append(result[1])
        print("{} simulations completed".format(i+1))
    ...
    fig, ax = plt.subplots(1, 2)
    fig.suptitle("n = {}, {} simulations".format(n, simulations))
    ax[0].set_xlabel("#triangles required for connecting the walls")
    ax[0].set_ylabel("frequencies")
    ax[0].hist(results1)

    ax[1].set_xlabel("#triangles required for complete coverage")
    ax[1].set_ylabel("frequencies")
    ax[1].hist(results2)

    print("Number of triangles required for connection = {}".format(sum(results1)/len(results1)))
    print("Number of triangles required for coverage = {}".format(sum(results2) / len(results2)))
